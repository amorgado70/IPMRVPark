...
namespace IPMRVPark.WebUI.Controllers
{
    public class ReservationController : Controller
    {
        ...
        private List<SelectionOption> SearchByNameOrPhoneList(string searchString)
        {
            //Return value
            List<SelectionOption> results = new List<SelectionOption>();
            if (searchString != null)
            {
                ...
                if (searchString.Any(char.IsDigit))
                {
                    ...
                    foreach (customer_view customer in allCustomers)
                    {
                        ...
                        {
                            results.Add(new SelectionOption(customer.id, customer.fullName + " - Phone: " + customer.mainPhone));
                        }
                        if (results.Count() > 5)
                        {
                            results.Add(new SelectionOption(-1, "..."));
                            return results;
                        }
                    };
                }
